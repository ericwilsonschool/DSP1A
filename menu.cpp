...
void readFile(ifstream &fin, list<Assignment> &completed,
              list<Assignment> &assigned) {
  // Get line, parse info, add to appropriate list
  string dueDate, description, assignedDate, status;
  string rawString;
  // Maybe insert a thing that clears out the lists just in case there's stuff
  ...
  while (!fin.eof()) {
    ...
    {
      Assignment newAssignment(newAss, newDue, description,
    ...
    {
      cout << "File read unsuccessful!\nCould not parse item with status "
           << status << "!\n";
      return;
    }
  }
...
void printList(list<Assignment> assList) // teehee
{
  ...
  if (assList.empty()) {
    ...
    return;
  } else { // print collumn headers
    cout << "ASSIGNED   DESCRIPTION             DUE       COMPLETED  STATUS\n";
    // iterate through list printing and stuff.
    for (list<Assignment>::iterator iter = assList.begin();
         iter != assList.end(); ++iter) {
      // Print assigned date and description
      cout << ' ' << iter->assigned << "   " << setfill(' ') << left << setw(24)
           << iter->description << setw(10) << iter->dueDate;
...
void parseInput(string &dueDate, string &description, string &assignedDate,
...
// assAssignment is dual function. Behaves differently depending on bool
// parameter.
void addAssignment(list<Assignment> &theList, bool isCompleted) {
  ...
  if (theList.empty() /*|| newAssignment.dueDate > (theList.end()).dueDate*/) {
    ...
    for (list<Assignment>::iterator iter = theList.begin();
         iter != theList.end();) {
...
void save(string fileName, list<Assignment> assigned,
          list<Assignment> completed) {
  ofstream fout;
  fout.open(fileName);
  ...
  for (list<Assignment>::iterator iter = completed.begin();
    ...
    fout << iter->assigned.toFileString() << ", " << iter->description << ", "
...
void checkLate(list<Assignment> completed) {
  ...
  for (list<Assignment>::iterator iter = completed.begin();
    ...
    if (iter->status == Assignment::Status::LATE) {
      cout << ' ' << left << setw(11) << iter->assigned << left << setw(24)
           << iter->description << left << setw(10) << iter->dueDate << '\n';
      ++count;
    }
  }
